function getP(u,p,code){
    ...
    $e.Encryption = function() {
        ...
        function getEncryption(password, salt, vcode, isMd5) {
            vcode = vcode || "";
            password = password || "";
            var md5Pwd = isMd5 ? password : md5(password), h1 = hexchar2bin(md5Pwd), s2 = md5(h1 + salt), rsaH1 = $e.RSA.rsa_encrypt(h1), rsaH1Len = (rsaH1.length / 2).toString(16), hexVcode = q.TEA.strToBytes(vcode.toUpperCase()), vcodeLen = "000" + vcode.length.toString(16);
            while (rsaH1Len.length < 4) {
                rsaH1Len = "0" + rsaH1Len
            }
            q.TEA.initkey(s2);
            var saltPwd = q.TEA.enAsBase64(rsaH1Len + rsaH1 + q.TEA.strToBytes(salt) + vcodeLen + hexVcode);
            q.TEA.initkey("");
            __monitor(488358, 1);
        ...
        function getRSAEncryption(password, vcode, isMd5) {
            ...
            var str3 = $e.RSA.rsa_encrypt(str2);
            return str3
        }
    ...
    return $e.Encryption.getEncryption(p,uin2hex(u),code);
}